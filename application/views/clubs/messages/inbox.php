...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(document).on('click','#action-delete-email',function(){
              ...
              if (selectedEmails.length > 0) {
                  ...
                  if(confirm("Do you want to delete this email? there is no UNDO")) {
                      ...
                      $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/inbox_message_delete/', {'emails' : selectedEmails}, function(res){
        ...
        $(document).on('click', '.reply-btn', function(){
            ...
            if(message != "") {
                ...
                $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/inbox_reply_email/', {'message': message, 'toID': emailID, 'fromID': fromID}, function(res){
        ...
        $(document).on('click', '.action-delete-single-email', function(e){
            ...
            if(confirm("Do you want to move this message to trash?")) {
                ...
                $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/inbox_single_email_trash/', {'email' : emailID }, function(res){
        ...
        $(document).on('click', '.item-row-checkbox', function(e){
            ...
            $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/inbox_set_mark_status/', {'email': emailID, 'mark': markStatus}, function(res){
    ...
    function loadInboxPartial(elem, search) {
        ...
        elem.load('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/inbox_partial/' + search, function(res){
            hideLoader();
        });
    }

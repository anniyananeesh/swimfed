...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(document).on('click','#action-delete-email-trash',function(){
              ...
              if (selectedEmails.length > 0) {
                  ...
                  if(confirm("Do you want to delete this email? there is no UNDO")) {
                      ...
                      $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/trash_message_delete/', {'emails' : selectedEmails}, function(res){
        ...
        $(document).on('click', '#action-move-inbox-trash', function(e){
            ...
            if (selectedEmails.length > 0) {
              ...
              $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/trash_move_inbox/', {'emails' : selectedEmails}, function(res){
        ...
        $(document).on('click', '.action-delete-single-email-trash', function(e){
            ...
            if(confirm("Do you want to move this message to trash?")) {
                ...
                $.post('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/trash_single_email_delete/', {'email' : emailID }, function(res){
    ...
    function loadInboxPartial(elem, search) {
        ...
        elem.load('<?php echo HOST_URL . '/Async/' . CLUB_URL ?>/trash_partial/' + search, function(res){
            hideLoader();
        });
    }
